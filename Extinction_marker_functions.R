...
#' meta_nalk_raw <- read.xlsx("Extinction_Marker_Metadata.xlsx", sheet = "Sheet1")
#'
...
quantify_compounds <- function(a_data, sheet) {
    ...
    meta_raw <- read.xlsx("Raw_data/Extinction_Marker_Metadata.xlsx", sheet = sheet) # import metadata

    meta <- meta_raw[order(match(meta_raw$sample, rownames(a_data))), ] # order the metadata to the same order as the area data
...
# DEFINE A FUNCTION FOR CUSTOMIZING PRELIMINARY PLOTS:
#   variables:
#   scale_x, scale_y: expand scale limits to 0
#   y_name: label y axis
#   extinction horizon: horizontal line at extinction horizon
plot_common_parameters <- function(scale_x = c(0, 0), scale_y = c(0, 0), x_name, y_name, extinction_horizon = TRUE, axis_ticks = "black", rev_ax = TRUE) {
  theme <- theme(axis.text.x = element_text(vjust = 0.5, hjust = 0.5, size = 14, color = axis_ticks), # rotates x axis by 90 degree, labels in middle of tick (vjust)
  ...
  extinction_horizon_gg <- geom_vline(xintercept = 0, colour = "brown1", linetype = "dashed")
